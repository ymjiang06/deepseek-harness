...
`openai-completions` 请求会通过 pi-ai 的 `onPayload` 钩子传递函数工具 schema。适配器在序列化前，为每个缺失或为 `null` 的对象型 JSON Schema 节点补上 `required: []`，包括嵌套对象属性。该处理只改变出站请求；已有的 `required` 数组和其他协议保持不变，同时兼容那些会把省略字段解码为 `null`、并要求该字段必须是数组的网关。
