...
`openai-completions` requests pass function tool schemas through pi-ai's `onPayload` hook. Before serialization, the adapter adds `required: []` to every object-valued JSON Schema node whose `required` field is absent or `null`, including nested object properties. This changes only the outbound payload; existing `required` arrays and other protocols remain unchanged, while gateways that decode an omitted field as `null` receive the array they require.
